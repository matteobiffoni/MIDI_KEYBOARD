...
// Lookup table for the default CRC-8 polynomial (see at the bottom of this file)
...
static inline crc_t crc_division_round(crc_t current_crc, uint8_t byte) {
    crc_t _crc = current_crc ^ byte;
#if CRC_POLY == 0x07 && CRC_INIT == 0x00
    _crc = crc8_table_0x07[_crc];
#else
    for(uint8_t k = 0; k < 8; k++) {
        if(_crc & 0x80)
            _crc = (_crc << 1) ^ CRC_POLY;
        else _crc <<= 1;
    }
#endif
    return _crc;
